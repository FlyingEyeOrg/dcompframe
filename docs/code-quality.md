...
5. 测试覆盖率：通过（核心行为均有单测/集成测试，当前 x64 Debug 为 59/59 通过）。
...
## 本轮 Flex 布局与命中测试门禁（2026-04-10）

1. 注释完整性：通过（Flex line 拆分和 pointer route 逻辑有明确函数边界）。
2. C++20 规范：通过（新增实现继续遵循智能指针、值对象和 `std::clamp` / `std::optional` 风格）。
3. 内存/资源泄露：通过（事件对象为栈内临时值；命中测试不引入额外所有权环）。
4. 可读性：通过（`FlexPanel` 与 `StackPanel` 兼容层职责分离明确）。
5. 测试覆盖率：通过（新增 4 条布局/命中测试，全量 `58/58` 通过）。
6. 性能：通过（hit-test 递归和 flex line 分配均为当前树规模下可接受复杂度）。
7. 安全性：通过（pointer route 统一走元素树，减少窗口消息层的分散分发分支）。
8. SOLID/DRY：通过（`StackPanel` 复用 `FlexPanel` 实现，不再维护两套主轴分配代码）。
9. 错误处理：通过（demo 目标构建成功，测试通过；本轮未引入新的静默 fallback）。

## 本轮 Flex-only 收口与自定义标题栏门禁（2026-04-10）

1. 注释完整性：通过（新增 helper 聚焦 caption、Badge/Radio/Toggle 渲染和命中逻辑，函数边界清晰）。
2. C++20 规范：通过（保持值对象、智能指针和显式状态机风格，无额外运行时脚本依赖）。
3. 内存/资源泄露：通过（DWM/D2D/D3D 资源仍走显式释放与宿主生命周期，不新增悬挂所有权）。
4. 可读性：通过（render target 虽大，但主几何来源已统一到真实 bounds，复杂度较之前下降）。
5. 测试覆盖率：通过（构建成功，`ctest --preset vs2022-x64-debug-tests` 为 `57/57`）。
6. 性能：通过（页面级滚动改为统一 bounds 偏移；列表视口裁剪与滚动条逻辑未退化为全量遍历）。
7. 安全性：通过（caption/resize/DPI 路径按 Win32 官方消息语义处理，避免错误 frame 刷新与 DPI 矩形应用）。
8. SOLID/DRY：通过（旧布局入口删除后，主布局实现只剩一条 Flex 路径；新增控件复用统一状态模型和渲染 helper）。
9. 错误处理：通过（构建、测试、窗口状态切换、滚动偏移与命中边界均保留 clamp 或显式失败路径）。

## 本轮 DWM 兼容与 Demo 占位收口门禁（2026-04-11）

1. 注释完整性：通过（新增逻辑集中在系统 hit-test、DWM 属性设置和 demo 预留空间 helper，函数边界明确）。
2. C++20 规范：通过（继续使用值对象、智能指针和显式状态切换，无脚本式旁路修复）。
3. 内存/资源泄露：通过（未引入新的原始资源所有权；DWM 属性调用不持有额外资源）。
4. 可读性：通过（默认窗口路径与显式 no-redirection 路径职责已拆开，Demo 重叠责任也从布局引擎回收到示例层）。
5. 测试覆盖率：通过（新增 3 条窗口/标题栏回归，全量 `59/59` 通过）。
6. 性能：通过（resize 边界计算改为读取系统 metrics，仍是常量级；Demo padding/margin 调整不增加布局复杂度）。
7. 安全性：通过（caption/resize hit-test 与系统语义对齐，避免窗口管理行为回退；所有滚动/布局尺寸仍保留 clamp）。
8. SOLID/DRY：通过（窗口默认样式和特殊样式职责分离，标签占位问题在 demo 层修复，没有把示例问题继续压到布局引擎里）。
9. 错误处理：通过（构建、测试全部通过；未新增静默 fallback）。

## 本轮标题栏按钮与边框一致性门禁（2026-04-11）

1. 注释完整性：通过（新增逻辑集中在 caption 按钮状态机和符号绘制 helper，函数边界清晰）。
2. C++20 规范：通过（新增状态枚举与条件分支保持值语义，无宏式旁路）。
3. 内存/资源泄露：通过（仅新增按钮状态字段与 D2D 绘制调用，不引入额外资源所有权）。
4. 可读性：通过（标题栏点击链从系统 hit-test 迁移到客户端状态机后，职责更明确）。
5. 测试覆盖率：通过（更新集成测试以验证 `HTCLIENT` 契约和最小化/最大化点击，当前 `59/59` 通过）。
6. 性能：通过（caption 按钮绘制与状态判断均为常量级；Flex 单行交叉轴修复减少了无意义拉伸）。
7. 安全性：通过（窗口状态切换仍统一走 `WindowHost::set_window_state`，关闭动作仍通过 `WM_CLOSE`）。
8. SOLID/DRY：通过（caption 图标绘制集中到独立 helper，避免三按钮重复维护不同路径）。
9. 错误处理：通过（按下态在 `WM_CAPTURECHANGED` / `WM_CANCELMODE` / `WM_LBUTTONUP` 下统一清理，未保留悬挂状态）。
