...
5. `WindowHostTests.DefaultConfigPreservesTaskbarWindowWithoutNoRedirectionBitmap`
6. `WindowHostTests.ExplicitConfigCanStillEnableNoRedirectionBitmap`
7. `WindowHostTests.ResizeAndDpiChangeTriggerRedraw`
8. `WindowHostTests.StateTransitionsAreTracked`
9. `WindowHostTests.CreateMessageLoopAndDestroy`
10. `WindowHostTests.MessageLoopExitsWhenQuitPosted`
11. `WindowHostTests.DestroySingleWindowDoesNotQuitWhenOtherWindowAlive`
12. `WindowHostTests.DestroyNotificationClearsInternalState`
13. `WindowTests.SkeletonWindowInitializesWithoutDemoControls`
14. `ApplicationTests.InitializeDoesNotCreateWindowUntilRequested`
15. `UIElementTests.VisualTreeAddAndRemoveChild`
16. `UIElementTests.EventDispatchSupportsCaptureTargetAndBubble`
17. `UIElementTests.HitTestFindsDeepestVisibleDescendant`
18. `UIElementTests.DirtyAndFocusFlagsPropagateCorrectly`
19. `FlexPanelTests.ColumnArrangeRespectsGapAndStretch`
20. `FlexPanelTests.HorizontalWrapMovesOverflowItemsToNextLine`
21. `FlexPanelTests.ArrangePreservesParentOffsetAndRespectsMarginAndPadding`
22. `LayoutPanelsTests.NestedPanelsArrangeChildrenRecursively`
23. `FlexPanelTests.FlexGrowConsumesRemainingVerticalSpace`
24. `FlexPanelTests.RowLayoutDistributesGrowBasisAndGap`
25. `FlexPanelTests.WrapMovesOverflowItemsToNextLine`
26. `FlexPanelTests.OrderAndMinMaxConstraintsProduceStableBounds`
27. `InputManagerTests.HitTestRoutingDispatchesCaptureTargetAndBubble`
28. `FlexPanelTests.MeasureAggregatesWrappedRowsUsingLargestCrossContribution`
...
44. `LayoutManagerTests.FlexStrategyAppliesSequentialBounds`
...
53. `IntegrationTests.WindowRenderTargetReturnsSystemCaptionAndResizeHitTests`
54. `IntegrationTests.DeviceLossRecoveryStressLoopRemainsStable`
55. `IntegrationTests.WindowRenderTargetProcessesTextInputAndConsumesComboWheel`
56. `IntegrationTests.WindowRenderTargetProcessesTabExpanderAndScrollbarTrackClicks`
57. `ReliabilityTests.SoakBaselineLoopMaintainsConsistency`
58. `ReliabilityTests.ResourcePeakPatrolStaysBoundedAfterReleaseCycles`
59. `ReliabilityTests.FaultInjectionCoversDeviceLostConfigMissingAndCorruptJson`
...
- 新增用例：
	- `WindowHostTests.DefaultConfigPreservesTaskbarWindowWithoutNoRedirectionBitmap`
	- `WindowHostTests.ExplicitConfigCanStillEnableNoRedirectionBitmap`
	- `IntegrationTests.WindowRenderTargetReturnsSystemCaptionAndResizeHitTests`
- 覆盖目标：
	- 锁定默认窗口扩展样式已回到 DWM 兼容路径。
	- 保留 `WS_EX_NOREDIRECTIONBITMAP` 的显式启用能力。
	- 锁定自定义标题栏的 `HTMAXBUTTON/HTCAPTION/HTRIGHT` 命中结果，避免 Snap、拖拽和 resize 行为再次退化。
