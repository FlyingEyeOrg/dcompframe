...
- 窗口层：
	...
	- demo 当前采用外层 `StackPanel` section 栈组织，每类控件各占一个 section；只有双列展示区域使用 `GridPanel`
	...
	- 布局系统现已升级为 `measure + arrange` 双阶段，`UIElement` 支持 intrinsic size、`flex grow/shrink`，`StackPanel` 负责主轴空间分配，`GridPanel` 根据子元素测量结果求轨道尺寸
	- 旧 demo 公式布局已移除，当前 `demo/main.cpp` 是专门用于验证 `Panel + StackPanel + GridPanel` 的布局验证窗口
