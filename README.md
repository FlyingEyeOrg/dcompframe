...
- 扩展控件：`TextBox`、`RichTextBox`、`ListView`、`ScrollViewer`、`CheckBox`、`ComboBox`、`Slider`
...
- 后端选择需显式指定（`DirectX` 或 `Warp`），框架不做隐式切换。
- demo 启动场景允许显式降级到 `Simulated`，避免因图形后端探测失败导致程序直接退出。
