...
`Direct3D11 + DirectComposition + 自定义 DWM 标题栏` 技术路线。
...
- 窗口层：
	- `WindowHost`（创建/销毁、消息循环、状态管理、DPI、默认 `WS_EX_APPWINDOW`，可选 `WS_EX_NOREDIRECTIONBITMAP`）
	- 自定义 DWM 标题栏：caption 区绘制、Chrome 风格 caption 按钮、客户端点击状态机、窗口拖拽、resize hit-test、Snap/阴影保留、Per-Monitor V2 DPI
	...
	- DWM frame 扩展边距已统一到四边，非最大化窗口边框保持一致，不再出现单侧黑线
	- demo overlay 支持按钮交互、caption 按钮和 List 区域逐项 hover 高亮
	- Demo 为所有带标签控件显式预留顶部占位与容器 padding，避免标签绘制侵占相邻控件视觉区
	...
	- `FlexPanel` 已修正单行交叉轴过度拉伸，默认不会在非 `align-content: stretch` 场景下把整行控件强制拉满
	...
	- 布局系统现已升级为 `measure + arrange` 双阶段，`UIElement` 支持 intrinsic size、`flex grow/shrink`，`FlexPanel` 负责主轴空间分配
...
- 测试：59 个测试（x64 Debug 全部通过）
- 示例：`dcompframe_demo`（已覆盖完整模块链路，窗口持续运行直到关闭）
- 已修复 `WS_EX_NOREDIRECTIONBITMAP` 场景下的透明窗口问题；当前该扩展样式收敛为可选项，而非默认窗口路径。
